...
class AlignedDDPMScheduler():
    ...
    def cross_forward(
        ...
        # color alignment - Optimized version with memory management
        if self.with_projection:
            with torch.enable_grad():
                ...
                ref_points = torch.swapaxes(img_ref.reshape(self.cur_bsz, self.latent_dim, -1), 1, 2).clone().detach()
    ...
    # for inference - Memory optimized
    def step_backward(
            self,
            img_original: torch.FloatTensor,
        ...
        # color alignment - Memory optimized
...
def main(args):
    ...
    if args.dataset_name is not None:
        ...
        raise ValueError("You must specify a dataset name.")
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            # Clear memory at start of each training step
            if step % 10 == 0:  # Clear every 10 steps
                clear_memory()
            ...
            # Shuffle the image with memory optimization
            with torch.no_grad():
                if not args.train_on_rgb:
                    blur_factor = 3
            ...
            with accelerator.accumulate(model):
                ...
                loss = F.mse_loss(model_output.float(),
                                  diffused_output['target_output'].float())
            ...
            del diffused_output, temp_input, model_output, loss
            if args.model_type == 'UNet2DConditionModel':
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    # Memory-optimized testing with reduced frequency and simpler visualization
                    if global_step % args.save_images_steps == 0:
                        ...
                        with torch.no_grad():
                            try:
                                batch_t = next(it_test_dataloader)
                        ...
                        # Simplified inference with fewer steps
                        bsz_t = clean_images_t.shape[0]
                        inference_steps = min(10, args.ddpm_num_inference_steps)  # Reduce inference steps for memory
                        ...
                        # Simplified inference loop with memory management
                        step_size = max(1, NUMSTEPS // inference_steps)
                        for t in range(NUMSTEPS - 1, -1, -step_size):
                            if args.model_type == 'UNet2DModel':
                                with torch.no_grad():
                            ...
                            del model_output_t, diffused_output_t
                            if step % 5 == 0:  # Clear memory every few steps
                                clear_memory()
                        ...
                        with torch.no_grad():
                            ...
                            rgb_chamfer_dist_l1_accuracy = torch.mean(rgb_chamfer_dist[0] / 2. * 255., dim=1).mean().item()
                        ...
                        if args.logger == "wandb":
                            accelerator.get_tracker("wandb").log(
                                {
                                    "test_original_img": [wandb.Image(
                                        torchvision.utils.make_grid(clean_images_t[:4]),  # Only log first 4 images
                                        caption=f"Original")],
                                    "test_pred_img": [wandb.Image(torchvision.utils.make_grid(
                                        torch.clamp(pred_img_STEP[:4], min=-1., max=1.)),  # Only log first 4 images
                                        caption=f"Predicted - Chamfer: {rgb_chamfer_dist_l1_accuracy:.2f}")],
                                    "chamfer_distance": rgb_chamfer_dist_l1_accuracy,
                                    "epoch": epoch},
                                step=global_step,
                            )
                        
                        # Clean up evaluation tensors
                        del clean_images_t, pred_img_STEP, clean_images_t_points, pred_img_STEP_points
                        del rgb_chamfer_dist, x_t, c_t
                        clear_memory()
